...
## 2026-05-15 — Signals page never shows dates

**Decision:** Anonymous signals are rendered in random order, without any timestamp or "this cycle / last cycle" marker. This is a hard policy in both the database function (`get_cycle_signals` orders by `random()` and omits `submitted_at`) and every UI surface.

**Why:** The founder mockup showed dates on each signal (e.g. "28 Feb"). With a 9-respondent team — the smallest pilot size we support — any chronological marker on a comment makes the last submitter identifiable to leadership. The 8-respondent threshold protects against count-based identification but not against time-based. Date-on-signal would unwind the anonymity guarantee the entire product rests on.

The cost of this policy is a small loss of context (a leader can't tell whether sentiment is fresh or stale). The benefit — trust in the anonymity claim — is load-bearing. Revisit only if the minimum team size moves above ~20 respondents.

---

## 2026-05-15 — UI layout follows founder mockups, with one structural deviation

**Decision:** Build to the founder's mockups in shape and density: grouped sidebar with section headers, leadership/employee view-as toggle, user card at the bottom, hero stats card with response-rate progress, six category cards with status dot + description + colored progress bar, Key Unlocks auto-insight panel, and a What Needs Attention list with discussion/owner CTAs. Adopt the visual treatment but keep the product name "Purple Signals" (mockups labelled the design "Pulsor" — treated as the designer's working name).

The one deviation: a dedicated standalone `/signals` page rather than only an inline signals card. Reason: with the no-dates policy above, the signals card is less browsable than the mockup implies, and giving users a filterable per-category view is more useful than the small inline excerpt.

**Why:** The mockups are the demo target. Matching them gets us through the founder review and pilot conversations. The standalone Signals page is a small upward deviation that costs nothing and improves usefulness.

---

## 2026-05-15 — Pin the sidebar to the viewport, scroll only the main pane

**Decision:** The outer app shell uses `h-screen overflow-hidden` and the sidebar uses `h-screen sticky top-0`; the main content area carries `overflow-y-auto` and is the only scrolling surface.

**Why:** With `min-h-screen` and a regular flex layout, the sidebar grew with the page content — on long pages (`/trends`, `/growth/notes`) the user card and view-as toggle scrolled off the visible viewport, breaking the navigation pattern. The pinned-sidebar / scrolling-main layout is standard for app shells of this shape; the only thing to watch is internal scrolling regions inside main (the AI coach chat has its own `h-[calc(100vh-8rem)]`), which still behave correctly because they nest inside the scrollable main.

---

## 2026-05-15 — Demo seed pushed to production for the founder demo

**Decision:** Pushed the full local seed (10 users, 6 categories, 12 questions, 3 cycles, 216 responses, 6 articles) to the hosted Supabase project. Production dashboards therefore show real-looking data on first load.

**Why:** The immediate goal is a sharable demo URL the founder can hand to investors and pilot prospects. An empty production DB shows only the insufficient-responses state, which doesn't communicate what the product does. Demo data is the right call for now.

**Cost:** Before sharing the URL with a real pilot org, the demo users and their cycles must be removed from the production DB (or their passwords rotated). Anyone with the URL who knows `password` can sign in as `leader@demo.com` and see the whole dashboard. Tracked as the second item under Next Actions in STATUS.md.
